...
#     % and a random label in {-1,1}. The expected outcome is (very close to) log(2).
    w=np.zeros((5,1))
    logistic_loss = logistic(w,x[:,1].reshape((5,1)),np.array(random.choice([-1,1])).reshape(1,-1))[0]
    failtest = np.abs(logistic_loss - np.log(2)) > 2.2204e-15
    if failtest:
        r=r+1
        s.append('Test 2: Logistic function does not pass sanity check #2.')
    else:
        ok=ok+1
    ...
    if failtest:
        r = r+1
        s.append('Test 5: Hinge function does not pass checkgrad.')
    else:
        ok=ok+1
...
#     % norm of the gradient at the optimal solution should be zero (< 1e-05).
    ...
    f = lambda w : squaredloss(w,x,y)
    w_trained = grdescent(f,np.zeros((D, 1)),stepsize = 1e-05,maxiter = 1000,tolerance=1e-09)
    d = np.linalg.norm(w_trained)
    print(d)
    failtest = d > 1e-5

    if failtest:
        r = r+1
        s.append('Test 6: squared loss function does not pass checkgrad.')
    else:
        ok=ok+1
    print('Completed Test 6\n')
#
#
